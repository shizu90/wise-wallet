...
public class ResetReminderCommandHandlerTests {
    @Mock
    private ReminderRepository reminderRepository;
    @Autowired
    @InjectMocks
    private ResetReminderCommandHandler resetReminderCommandHandler;
    ...
    @Test
    @DisplayName("Reset reminder command handler test")
    void resetReminder() {
        Reminder reminder = populate();
        ResetReminderCommand command = new ResetReminderCommand(reminder.getId());

        Mockito.when(reminderRepository.load(command.getAggregateId(), null)).thenReturn(Optional.of(reminder));

        Reminder returnedReminder = resetReminderCommandHandler.handle(command);

        Assertions.assertEquals(0L, returnedReminder.getCurrentRuns().getValue());
    }
}
